...
ap.add_argument("-i", "--input",help="path to input video", default = "./test_video/b1.mp4")
...
def main(yolo):
    ...
    # Define the codec and create VideoWriter object
        ...
        out = cv2.VideoWriter('./output/'+args["input"][12:15]+ "_" + args["class"] + '_output.avi', fourcc, 15, (w, h))
        list_file = open('detection_'+ args["input"][13:15] +'.txt', 'w')
        frame_index = -1
    ...
    while True:
        ...
        if writeVideo_flag:
            ...
            if len(boxs) != 0 and len(indexIDs) > 0:
                for i in range(0,len(indexIDs)):
                    #print(j)
                    print(indexIDs)
                    list_file.write(str(frame_index)+',')
                    list_file.write(str(indexIDs[i])+',')
